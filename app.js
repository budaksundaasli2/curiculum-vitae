...
// --------- Skills Chips (Enter to create new chip) ---------
(function setupSkillsChips(){
	...
	document.addEventListener('keydown', (e)=>{
		if(e.key !== 'Enter' || e.shiftKey) return;
		if(!e.target || !e.target.isContentEditable) return;
		const ul = e.target.closest('ul.skills-grid');
		if(!ul || !skillsRoot.contains(ul)) return;
		e.preventDefault();
		let currentLi = e.target.closest('li');
		const newLi = document.createElement('li');
		newLi.innerHTML = '<br>';
	...
	document.addEventListener('input', (e)=>{
		const ul = e.target && e.target.closest && e.target.closest('ul.skills-grid');
		if(ul && skillsRoot.contains(ul)) sanitizeChips(ul);
	});
})();